...
def sample_pdf(bins, weights, N_samples, det=False, pytest=False):
    ...
    cdf = torch.cumsum(pdf, -1).to('cuda')
    cdf = torch.cat([torch.zeros_like(cdf[...,:1]), cdf], -1)  # (batch, len(bins))

    # Take uniform samples
    if det:
        u = torch.linspace(0., 1., steps=N_samples).to('cuda')
        u = u.expand(list(cdf.shape[:-1]) + [N_samples])
    else:
        u = torch.rand(list(cdf.shape[:-1]) + [N_samples]).to('cuda')
    ...
    below = torch.max(torch.zeros_like(inds-1).to('cuda'), inds-1)
    above = torch.min((cdf.shape[-1]-1) * torch.ones_like(inds).to('cuda'), inds)
...
def sample_from_lineseg(near, far, N_lines, N_samples, perturb=0., lindisp=False, pytest=False):
    ...
    t_vals = torch.linspace(0., 1., steps=N_samples).to('cuda')
