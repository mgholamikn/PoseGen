...
def batchify_rays(rays_flat, chunk=1024*32, ray_caster=None, **kwargs):
    ...
    for i in range(0, rays_flat.shape[0], chunk):
        batch_kwargs = {k: kwargs[k][i:i+chunk].to('cuda') if torch.is_tensor(kwargs[k]) else kwargs[k]
                        for k in kwargs}

        ray_caster=ray_caster.to('cuda')            
        ret = ray_caster(rays_flat[i:i+chunk].to('cuda'), **batch_kwargs)
        for k in ret:
            if k not in all_ret:
                all_ret[k] = []
