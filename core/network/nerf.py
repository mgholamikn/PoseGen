...
class NeRF(nn.Module):
    ...
    def raw2outputs(self, raw, z_vals, rays_d, raw_noise_std=0, pytest=False,
        ...
        dists = torch.cat([dists, torch.Tensor([1e10]).expand(dists[...,:1].shape).to('cuda')], -1)  # [N_rays, N_samples]
        ...
        disp_map = 1./torch.max(1e-10 * torch.ones_like(depth_map).to('cuda'), depth_map / (torch.sum(weights, -1)  + 1e-10))

        invalid_mask = torch.ones_like(disp_map).to('cuda')
        invalid_mask[torch.isclose(weights.sum(-1), torch.tensor(0.).to('cuda'))] = 0.
        disp_map = disp_map * invalid_mask

        acc_map = torch.minimum(torch.sum(weights, -1), torch.tensor(1.).to('cuda'))
